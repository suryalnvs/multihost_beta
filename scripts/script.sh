...
createChannel() {
	setGlobals 0
        ...
        do
			...
			peer channel create -o orderer0.example.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/mychannel0.tx -t 60 >&log.txt
		else
		#	peer channel create -o orderer0.example.com:7050 -c $CHANNEL_NAME$i -f ./channel-artifacts/mychannel$i.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -t 60 >&log.txt
                        peer channel create -o orderer0.example.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/mychannel0.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -t 60 >&log.txt
		fi
                sleep 10
		res=$?
		cat log.txt
		verifyResult $res "Channel creation failed"
		echo "===================== Channel \"$CHANNEL_NAME$i\" is created successfully ===================== "
		echo
	done
}
